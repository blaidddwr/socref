...
)
{
    ...
    {
        throw FileError(QObject::tr("Failed opening %1: %2").arg(path,file.errorString()));
    }
    ...
    {
        throw WriteError(
            QObject::tr("Failed writing project file %1: %2").arg(path,file.errorString())
        );
    }
...
)
{
    ...
    if (!dir.exists())
    {
        if (!dir.mkpath("."))
        {
            throw FileError(QObject::tr("Failed creating directory %1.").arg(_path));
        }
    }
    if (!dir.isReadable())
    {
        throw FileError(QObject::tr("The directory at %1 is not readable.").arg(_path));
    }
    _open = true;
}
}
