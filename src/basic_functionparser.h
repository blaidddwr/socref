...
namespace Basic
{
    ...
    * will grab input from the source file if the matching function is found,
    * taking all lines of that function. These input lines are then parsed, cutting
    * off any additional indent spacing and adding inline comments as needed. Steps
    * of operation are provided for filling in the inline comments.
    *
    * The function is matched in the source file based off a header regular
    * expression that is built with provided methods of this class. Once matched
    * lines will be read as input until the last out token is found signaling the
    * end of the function. The first header line defining the function and last
    * line with the final out token will be included in input.
    *
    * Additional parsing is done with the input lines by looking for marker lines
    * of inline comments. The provided comment begin string is matched with the
    * beginning of the string, excluding any preceding white space. Each
    * consecutive lines of inline comment matches will be treated as a single
    * inline comment and filled in with the specific operation in the provided
    * steps of operation. The order of inline comments will be matched with the
    * order provided in the steps of operation.
    */
   ...
   private:
      ...
      /*!
       * The header or full name of this function that will be used as the first line
       * of output for this function parser.
       */
      const QString _header;
      /*!
       * Steps of operation for this function. This is used as inline comments,
       * following the same order as inline comments are found when parsing the input
       * lines.
       */
      ...
      bool _first {true};
      /*!
       * The maximum number of spaces that are cut off from the beginning of each
       * input line for this function parser. Only white space is cut off the
       * beginning of input lines.
       */
      int _cutOff {0};
      /*!
       * The current execution block depth as this function parser is reading in input
       * lines. This is used to determine when the function ends in this parent
       * scanner's source file.
       */
      int _depth {0};
      /*!
       * True when the first input line of a single inline comment block has been read
       * or false otherwise. This is used to add only one step of operation per block
       * of inline comment input lines.
       */
      bool _edgePast {false};
      /*!
       * Index to the next step of operation to be used to generate an inline comment
       * for this function parser.
       */
