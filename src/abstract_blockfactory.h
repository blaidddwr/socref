...
namespace Abstract
{
    ...
    * each block type such as its display and element names. The two element name
    * functions must mirror one another in an implementation or undefined behavior
    * will occur.
    */
   class BlockFactory
   {
   public:
       ...
       * Block types must be defined for all integers from 0 to one less than the
       * number of block types returned by this interface.
       *
       ...
       *                    found.
       *
       * @return The integer type of the block with the given element name if found or
       *         -1 if no block type is found with that element name.
       */
      ...
      /*!
       * This interface creates and returns a new block of the given type. The new
       * block is expected to have either a null data or new default state, depending
       * on the given flag.
       *
      ...
      /*!
       * This interface creates and returns a new root block. The root block is used
       * as the root block for all projects of this project type. The new root block
