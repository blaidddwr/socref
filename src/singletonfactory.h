 ...
 * factory. A singleton class is a class that has only one global instance for 
 * the entire program. The global instance can be set to a new instance at 
 * anytime, deleting any existing instance. The is most commonly used with 
 * abstract factories that have a single global instance in a program. 
 *
...
/*!
 * Returns the single global instance of the singleton factory that inherits 
 * from this template class. If an instance has never been set then an exception 
 * is thrown. 
 *
...
/*!
 * Sets a new instance of the singleton factory that inherits this template 
 * class with the given pointer. If an instance already exists the old one is 
 * deleted and replaced by the new one given. 
 *
