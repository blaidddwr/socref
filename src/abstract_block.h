...
{
   /*!
    * This represents a single block in this application's reference system. The
    * block system is the core function of this application, representing blocks as
    * a single atomic item for referencing code. The block system conforms to a
    * parent child relationship. Virtual interfaces are provided for basic
    * information, spawning view and abstract edit widgets. Interfaces are also
    * provided for reading from and writing to XML elements.
    *
    * The block system represents all elements of code structure in a reference.
    * Individual implementations should be defined for different code objects such
    * as variables or functions. They should also be defined for modifiers or
    * additions to other blocks, such as arguments to a function or methods to a
    * class.
    *
    * Each block can have a single parent and any number of children. The children
    * are contained in a ordered list to allow the entire block structure to be
    * viewed in this application's block view widget. This allows for easy
    * mimicking of most code structures that also follow this relationship, such as
    * arguments of a function or methods of a class.
    *
    * The virtual interfaces for this class fall into three primary groupings. The
    * first provides the system with basic information usually required to display
    * the block in the block view. The second deals with spawning view and abstract
    * edit widgets to have a detailed view of the block or edit its data,
    * respectively. The final deals with reading from and writing to XML elements
    * used for saving projects to an XML file. Optional interfaces are also
    * provides for dealing with child events, such as children being added or
    * removed.
    *
   ...
   public:
      ...
      /*!
       * This interface creates and returns an abstract edit GUI dialog that provides
       * the ability to edit this block's data.
       *
   ...
   signals:
      /*!
       * Signals that this block has been updated and any view attached to it must
       * update this block's display.
       */
      void updated();
      /*!
       * Signals that a child block of this block has been updated. The given child
   ...
   protected:
      ...
      /*!
       * This interface creates and returns a new block object of this block's type
       * with no data and returns a pointer to the new block.
       *
...
{
   /*!
    * creates and returns a list of this block's children that matches the given
    * type. The returned list has no other copies.
    *
