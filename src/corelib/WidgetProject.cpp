...
{
    if (auto blockWidget = qobject_cast<AbstractBlockWidget*>(blockScrollArea()->widget()))
    {
...
)
{
    ...
    if (auto block = qobject_cast<AbstractBlockWidget*>(widget))
    {
        auto applyButton = blockApplyButton();
        connect(applyButton,&QPushButton::clicked,block,&AbstractBlockWidget::apply);
        connect(block,&AbstractBlockWidget::modifiedChanged,applyButton,&QWidget::setEnabled);
        applyButton->setEnabled(block->modified());
        auto codeButton = blockCodeButton();
        connect(codeButton,&QPushButton::clicked,block,&AbstractBlockWidget::showCode);
        connect(
            block
            ,&AbstractBlockWidget::codeChanged
            ,applyButton
