#ifndef BASIC_BLOCKFACTORY_H
#define BASIC_BLOCKFACTORY_H
#include <QDomElement>
...
{
   /*!
    * This mostly implements the abstract block factory. This facilitates reading
    * in a XML configuration file that defines all basic blocks types this factory
    * can produce. The definition of each basic block type is provided as a root
    * level XML element.
    *
    * All interfaces from the abstract block factory are implemented except for the
    * element name interface. This must be implemented by the actual project type
    * and is used by this interface to make sure any enumeration it contains
    * matches the XML file. The order of block types within any enumeration must
    * match the order in the XML configuration file this basic block factory loads.
    *
    * A new interface is also provided for creating implementation classes of the
    * basic block class. Basic blocks do no require to implemented and can simply
    * be generic basic blocks. To do this simply return a null pointer with this
    * class's interface.
    */
   class BlockFactory : public Abstract::BlockFactory
   {
   public:
   ...
   private:
      ...
      /*!
       * List of element names for this factory's list of basic block type
       * definitions.
       */
      ...
      /*!
       * Index to the basic block type definition that is considered the root block
       * type for this factory's implementation's project type.
       */
      int _rootType {-1};
      /*!
       * True if this factory has been checked for consistency with its implementation
       * or false otherwise.
       */
