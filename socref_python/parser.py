...
class Parser(abstract.AbstractParser):
    ...
    def __buildPaths_(self, parent, path):
        ...
        """
        for block in parent:
            if block.dirName() is not None:
                self._addPath_(block,os.path.join(path,block.dirName(),"__init__.py"))
                self.__buildPaths_(block,os.path.join(path,block.dirName()))
            elif block.fileName() is not None:
                self._addPath_(block,os.path.join(path,block.fileName()))
